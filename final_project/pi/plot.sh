...
set style line 5 lt 1 linecolor rgb "#ff00ff" lw 5
set yrange [-20:100]
...
set ylabel '%'
plot '~/data.dat' u 1:7 title 'water level (%)' w i linestyle 5, '~/data.dat' u 1:3 title 'battery level (%)' w l linestyle 4
unset multiplot
quit
EOF
NEWINDEX=`awk -F, '{printf("%d\n", $1 + 1)}' ~/index`
echo "${NEWINDEX}" > ~/index
