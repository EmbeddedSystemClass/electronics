...
uint16_t        curr_light = 0;     //Global variable to get lum at time -T-
...
void    __ISR(_ADC_VECTOR, IPL4AUTO)  ADC1_Interrupt()
{
        ...
        light_level = (uint16_t)ADC1BUF0;       //Get ADC1BUF0
        if ((curr_light - light_level) > 60)    //Diff for Screen Activation
        {
            foo();                              //Function for Screen Active
        }
        curr_light = light_level;               //Save Lum
...
{
	display_write_str("    ", 0, 6);
	display_write_dec(curr_light, 0, 6);
}
