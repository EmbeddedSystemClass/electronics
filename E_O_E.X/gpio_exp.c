...
void	gpio_exp_write_byte_to_reg(uint8_t register_addr, uint8_t data)
{
	...
	if(gpio_exp_write_byte(0x40)){led_alert(0xe000);}		//slave address
	if(gpio_exp_write_byte(register_addr)){led_alert(0xe000);};	//register to write
	if(gpio_exp_write_byte(data)){led_alert(0xe000);};		//data to write
